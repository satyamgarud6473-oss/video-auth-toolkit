...
import cv2, numpy as np, hashlib, os, tempfile
...
def get_video_basic_info(path):
    ...
    cap.release()
    return {"fps": float(fps), "width": w, "height": h, "frame_count": frame_count, "duration_sec": float(duration_sec)}
...
def extract_audio_waveform(video_path):
    """
    Extract mono 16k audio using pydub (requires ffmpeg binary on server).
    Returns (wav_path, sample_rate, samples ndarray [float32]).
    """
    try:
        audio = AudioSegment.from_file(video_path)
        audio = audio.set_channels(1).set_frame_rate(16000)
        samples = np.array(audio.get_array_of_samples()).astype(np.float32)
        samples = samples / (2**15)  # normalize 16-bit
        sr = audio.frame_rate
